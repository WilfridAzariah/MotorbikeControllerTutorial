...
{
    public Vector3 acceleration, lastVelocity;
    ...
    void Awake()
    {
        ...
        foreach (WheelComponent w in wheels)
        {
			...
			// Debug.Log("Wheel Radius = " + bikeWheels.setting.Radius);
    ...
    void FixedUpdate()
    {
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            w.rotation = Mathf.Repeat(w.rotation + col.rpm * 360.0f / 60.0f, 360.0f);
...
//        Pitch = Mathf.Clamp(1.2f + ((motorRPM - bikeSetting.idleRPM) / (bikeSetting.shiftUpRPM - bikeSetting.idleRPM)), 1.0f, 10.0f);
		...
		Debug.Log("Before end of FixedUpdate()");
		
		Debug.Log("myRigidbody.velocity = " + myRigidbody.velocity);
		
		Debug.Log("myRigidbody's acceleration = " + acceleration);
		
		
		Debug.Log("End of FixedUpdate()");
